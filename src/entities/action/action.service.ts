...
export class ActionService {
    ...
    async create(createActionDto: ActionCreateDto): Promise<Action> {
        const { description, time, userTournamentTimeId, taskId } =
            createActionDto;

        const multilingualTextInstance =
            await this.multilingualTextService.create(description);
        const userTournamentTimeInstance =
            await this.userTournamentTimeService.getInstance(
                userTournamentTimeId
            );
    ...
    async maxActionCountInOnOneTournament(
        taskId: number,
        listOfTournamentsIdsOfGivenUser: number[]
    ): Promise<number> {
        const counts = await this.actionRepository
            ...
            .andWhere(
                "action.userTournamentTimeId IN (:...tournamentTimeIds)",
                { tournamentTimeIds: listOfTournamentsIdsOfGivenUser }
            )
